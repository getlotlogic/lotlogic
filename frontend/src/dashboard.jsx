...
function useNowTick(everyMs = 30000) {
...
const LOT_TIMEZONE = 'America/New_York';
...
function tzOffsetOn(ymd, timeZone = LOT_TIMEZONE) {
  try {
...
function tzOffsetAt(date, timeZone = LOT_TIMEZONE) {
  try {
    const parts = new Intl.DateTimeFormat('en-US', { timeZone, timeZoneName: 'longOffset' })
      .formatToParts(date);
    const name = (parts.find(p => p.type === 'timeZoneName') || {}).value || '';
...
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im56ZGtvb3VvYWVkYmJjY3Jhb3RpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzMxMzg2OTQsImV4cCI6MjA4ODcxNDY5NH0.WrlTCKEmmziBUX1E9vBmBpHRSg7_RRKBLIxSPKy189E';
...
function _supabaseFetch(input, init) {
  if (_supabaseToken) {
...
function applySupabaseAuth(token) {
...
function useActiveRoster(propertyId) {
...
function useIntervalFetch(fn, ms, deps) {
  useEffect(() => {
    let cancelled = false;
    const run = async () => {
      if (cancelled) return;
...
function bundleVehicleEvents(plateEvents, verifiedPairs = []) {
...
function filterEvidencePackages(bundles) {
...
function NavIconJobs() {
  return <div className="nav-icon"><svg viewBox="0 0 24 24"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg></div>;
}
function NavIconLots() {
  return <div className="nav-icon"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg></div>;
}
function NavIconEarnings() {
  return <div className="nav-icon"><svg viewBox="0 0 24 24"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg></div>;
}
function NavIconAccount() {
  return <div className="nav-icon"><svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div>;
}
...
function ViolationProofModal({ violation, latestSnapshotUrl, latestDetections, cameraZones, matchedDetection, tunnelSnapshotUrl, onClose, onAutoGone }) {
...
function bboxZoneOverlap(bxMin, byMin, bxMax, byMax, zonePolygon) {
...
function ViolationSnapshot({ src, detections, matchedDetection, style, onClick, maxHeight, borderRadius }) {
  const [loaded, setLoaded] = useState(false);
...
function CrossCameraSightings({ plate, propertyId, when, excludeCameraIds = [], fmtDateTime }) {
...
function JobsPage({ lots, violations, alprViolations = [], loading, lotStates, onAction, isOwner, deepLinkViolationId, user, onNavigate }) {
...
function EarningsPage({ violations, lots, isOwner, user, onNavigate }) {
...
function InvoicesPage({ lots, partners: partnersProp, user, isOwner = true, isPlatformAdmin = false }) {
...
function ConfirmationReviewView({ user, lots, partnersProp }) {
...
function CooldownChip({ passId, priorFlagCount, registeredAt, priorEnd }) {
  const [open, setOpen] = React.useState(false);
...
function fmtCooldownDateTime(iso) {
  if (!iso) return '';
...
function PassPhotoStrip({ pass, propertyId, cache, setCache }) {
...
function TruckParkingLog({ propertyId, propertyType, payToParkEnabled = false, isOwner = false, mode = 'default' }) {
...
function FeedbackModal({ propertyId, onClose }) {
  const { addToast } = useToast();
...
function ApartmentPermits({ property }) {
...
function ALPRPropertyDetailPage({ propertyId, onBack, user }) {
...
function RegisteredDrill({ propertyId, propertyName, onBack }) {
  const { addToast } = useToast();
...
function scopePropsToPartner(props, user) {
...
function ALPRPropertiesPage({ user, impersonating = false }) {
  const { addToast } = useToast();
