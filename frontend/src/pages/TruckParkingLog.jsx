...
export function TruckParkingLog({ propertyId, propertyType, payToParkEnabled = false, isOwner = false, mode = 'default' }) {
  ...
  // Calendar days in the viewer's local time — NOT elapsed hours. A pass
  // registered yesterday at 1:42 PM must read "Yesterday" this morning, even
  // though fewer than 24 hours have passed (ported from lotlogic PR #235,
  // which fixed this on the pre-Wave-2.6 dashboard.html).
  const fmtDaysAgo = (s) => {
    if (!s) return '';
    const d = new Date(s);
    ...
    const days = Math.round((startOfToday - startOfThatDay) / (24 * 60 * 60 * 1000));
