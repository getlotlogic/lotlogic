  ...
  try {
      ...
      if (sidecar.ok && !sidecar.bestPlate) {
        ...
        if (!isHardSkip) {
          ...
          if (LOG_REJECTED) {
            ...
            const key = `${camera.property_id}/diagnostic/${dateStr}/${camera.api_key}-${epochMs}-rejected-${reason}.jpg`;
            let imageUrl: string | null = null;
            let imageError: string | null = null;
            const upRes = await r2(key, extracted.bytes);
            if (upRes.ok) imageUrl = upRes.url;
    ...
    if (surviving.length === 0) {
      ...
      if (LOG_REJECTED) {
        ...
        const epochMs = nowDate.getTime();
        const dateStr = nowDate.toISOString().slice(0, 10);
        const key = `${camera.property_id}/diagnostic/${dateStr}/${camera.api_key}-${epochMs}-rejected-pr_no_plate.jpg`;
        let imageUrl: string | null = null;
        let imageError: string | null = null;
        const upRes = await r2(key, extracted.bytes);
        if (upRes.ok) imageUrl = upRes.url;
